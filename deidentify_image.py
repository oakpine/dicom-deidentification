...
class Deidentify:
    ...
    def __init__(self):
        ...
        self.fields = [(0x0008,0x0014), (0x0008,0x0018), (0x0008,0x0050), (0x0008,0x0080), (0x0008,0x0081), (0x0008,0x0090), (0x0008,0x0092),
        ...
        self.date_fields = [(0x0010,0x0030), (0x0010,0x0032)]

    def east_detect(self, image):
        layerNames = [
    ...
    def combine_rects(self, coords):
        rects = []
        for c in coords:
            matched = False
            for r in range(len(rects)):
                if self.overlap(rects[r], c):
                    matched = True
    ...
    def find_sensitive(self, data, img, ds, rect):
    ...
    def censor_text(self, img, data, rect, i):
        ...
        left = data['left'][i]//self.scale 
        top = data['top'][i]//self.scale 
        right = left + data['width'][i]//self.scale 
        bottom = top +data['height'][i]//self.scale
    ...
    def deidentify_image(self, ds):
        ...
        coords = self.east_detect(img) # Detect text with EAST
        rects = self.combine_rects(coords) # Find sections of text
        ...
        for rect in rects:
            ...
            # Resize image
            self.scale = max(2000//img.shape[0], 2000//img.shape[1])
            cropped = cv2.resize(cropped, None, fx=self.scale, fy=self.scale)
            ...
            img = self.find_sensitive(data_nothresh, img, ds, rect)
            # img = find_sensitive(data_inv_nothresh, img, file, rect)
            img = self.find_sensitive(data, img, ds, rect)
            img = self.find_sensitive(data_inv, img, ds, rect)

        # Convert back to uint16 if necessary
        if type(ds.pixel_array[0][0]) == np.uint16:
            img = (img*factor).astype('uint16')
        elif type(ds.pixel_array[0][0]) == np.int16:
            print('y int')
